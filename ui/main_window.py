...
from PyQt5.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QPushButton,
                             ...
                             QTextEdit, QHeaderView, QButtonGroup, QMessageBox) # Removed delegate imports
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        # ... (This entire section is the same as before)
        main_widget = QWidget()
    ...
    def setup_logs_page(self):
        ...
        # --- CHANGE 1: Removed the delegate, simplified styling ---
        self.logs_table.setAlternatingRowColors(True)
        self.logs_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeToContents) # Timestamp
        self.logs_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeToContents) # Type
        self.logs_table.horizontalHeader().setSectionResizeMode(3, QHeaderView.Stretch) # Message
    ...
    # ... (setup_live_alerts_page, setup_viz_page, setup_model_page are unchanged)
    ...
    def setup_viz_page(self):
        ...
        header_container.addWidget(heading)
        header_container.addStretch()
        layout.addLayout(header_container)

        # 2. Add a stretch to push the graph down from the top
        layout.addStretch(1)

        # 3. Add the graph itself
        layout.addWidget(self.protocol_chart)

        # 4. Add another stretch to push the graph up from the bottom
        # The two stretches will balance and center the graph
        layout.addStretch(1)
    ...
    def update_dashboard_tables(self):
        # ... (This method is mostly the same, just removing the delegate call)
    ...
    def update_protocol_chart(self):
        # --- CHANGE 2: Stabilize the visualization graph ---
        ax = self.protocol_chart.axes
        ax.clear()

        # Define a fixed order and color scheme
        protocols = ['TCP', 'UDP', 'ICMP']
        colors = ['#3498db', '#2ecc71', '#f1c40f'] # Blue, Green, Yellow

        # Get counts for our fixed protocols, defaulting to 0 if not present
    ...
    def populate_logs_table(self):
        # --- CHANGE 1: Manually style the log type ---
    ...
    # ... (All model manager methods are unchanged)
    ...
    def start_training(self):
        if not self.captured_pcap_path: self.log_model_status("Error: No pcap file captured yet."); return
        self.train_button.setEnabled(False)
